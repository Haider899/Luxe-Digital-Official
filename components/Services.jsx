...
export default function Services({ onStartProject }) {
  ...
  return (
    <section id="services" className="relative py-16 sm:py-20 md:py-24 lg:py-32 px-4 sm:px-6 md:px-8 lg:px-0 bg-white overflow-hidden">
      <div className="container-custom w-full">
        <div className="flex flex-col gap-6 md:gap-8 mb-12 md:mb-16 lg:mb-24">
          <div className="w-full">
            <motion.h4 
              initial={{ opacity: 0, x: -20 }}
              whileInView={{ opacity: 1, x: 0 }}
              className="text-emerald-600 font-black text-[9px] sm:text-[10px] mb-3 sm:mb-4 uppercase tracking-[0.3em]"
            >
              Our Core Expertise
            </motion.h4>
            <motion.h2 
              ...
              className="text-3xl xs:text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black text-slate-900 leading-tight md:leading-[0.9] tracking-tighter max-w-4xl"
            >
              Specialised <span className="text-emerald-500">Solutions</span> <br className="hidden sm:block" />
              For Results.
            </motion.h2>
          </div>
          <motion.p 
            ...
            className="text-slate-500 font-bold max-w-lg text-xs sm:text-sm uppercase tracking-widest leading-relaxed"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {services.map((service, idx) => (
            <TiltCard 
              key={idx} 
              className={service.size === 'lg' ? 'sm:col-span-2 lg:col-span-2' : ''}
            >
              <motion.div 
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                ...
                className="h-full glass-premium p-6 sm:p-8 md:p-10 rounded-[30px] md:rounded-[40px] border border-slate-100 shadow-xl group cursor-pointer relative overflow-hidden flex flex-col justify-between min-h-[300px] sm:min-h-[350px] md:min-h-[400px] hover:border-emerald-300 transition-colors"
              >
                <div className="absolute inset-0 bg-gradient-to-br from-emerald-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                
                <div className="relative z-10">
                  <div className={`w-14 sm:w-16 h-14 sm:h-16 bg-gradient-to-br ${service.color} rounded-2xl flex items-center justify-center text-white mb-4 sm:mb-6 md:mb-8 shadow-lg shadow-emerald-500/20 group-hover:scale-110 group-hover:rotate-6 transition-all duration-500 flex-shrink-0`}>
                    <service.icon className="w-6 sm:w-7 md:w-8 h-6 sm:h-7 md:h-8" />
                  </div>
                  <h3 className="text-xl sm:text-2xl md:text-3xl font-black mb-3 sm:mb-4 text-slate-900 tracking-tight group-hover:text-emerald-600 transition-colors">{service.title}</h3>
                  <p className="text-slate-500 font-medium leading-relaxed mb-4 sm:mb-6 md:mb-8 max-w-sm text-sm sm:text-base">
                    {service.description}
                  </p>
                </div>

                <div className="flex items-center gap-2 text-slate-900 font-black text-[9px] sm:text-[10px] uppercase tracking-widest group-hover:gap-4 transition-all">
                  <span className="w-6 sm:w-8 h-[2px] bg-emerald-500"></span>
                  Explore Solution <ArrowRight className="w-3 sm:w-4 h-3 sm:h-4 flex-shrink-0" />
                </div>

                {/* Decorative background number */}
                <div className="absolute -bottom-10 -right-10 text-[120px] sm:text-[150px] md:text-[180px] font-black text-slate-900/[0.02] pointer-events-none select-none group-hover:text-emerald-500/[0.05] transition-colors">
                  0{idx + 1}
                </div>
              </motion.div>
            </TiltCard>
